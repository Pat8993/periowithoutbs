  ...
  component: () => {
    ...
    const handleNavigate = (targetModuleId: string, section?: string) => {
      if (targetModuleId === "home") {
        navigate({ to: "/" });
      } else if (targetModuleId === "module-3" && section) {
        setSectionId(section);
        window.location.hash = section;
      } else if (targetModuleId === "module-3") {
        setSectionId(null);
    ...
    return (
      <AccessGate>
        <ModulePage module={modules[2]} currentSectionId={sectionId} onNavigate={handleNavigate} />
      </AccessGate>
    );
  },
});
